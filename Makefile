...
build: fmt
	...
	go build -race ${LDFLAGS} ${GCFLAGS} -o examples/bin/blog-deploy-server cmd/blog-deploy-server/main.go
	go build -race ${LDFLAGS} ${GCFLAGS} -o examples/bin/blog-deploy cmd/blog-deploy/main.go
